...
3. **指導步驟 (第7步)：** 將您偽代碼中的`L_stab`明確地寫為我們設計的`L_physics`，以包含我們討論過的所有物理約束。
...
目前感觉上面的这些物理公式只是针对图像处理的公式，而纯粹的黑洞相关的物理公式并没有用到，这里我有个疑惑，因为目前我们的黑洞逆问题，将黑洞的物理观测值进行图片修复生成，如果加入纯粹的物理公式，是否与图片生成的 gap 过大，无法顺利引入，或是作用不大，但是单纯的图像处理又是否合理 

### 5.2 Gemini 答疑

> [!TIP]
>
> 因为我也不确定我的想法是不是对的，这里需要用 Gemini 进行辅助。
