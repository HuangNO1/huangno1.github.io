...
## 一、前言
因為我前幾篇記錄並分享了 DPS、DiffPir、PnP-DM 這三種針對後驗的算法，因為我目前也在做相關的論文工作，這篇文章我會先按照我自己的理解先去總結一下擴散模型（其實我 DDPM 那篇論文並沒有精讀細節，因為時間不太足夠，後面有時間再針對細節重讀），避免在工作上有歧義，接著給出這些方法之間的區別與改動，再給出現有的工作階段可以怎麼去修正。

> Blind-PnPDM 是我讀錯的論文，一開始看錯了，我以為是 PnP-DM，讀到一半才知道不對勁，但是既然都讀了還是把事情做了收尾。
...
我在一開始理解錯了，以為PnP是包含先驗的部分，但隨著論文讀多了，其實這裡理解是錯誤的。
...
> 也就是說在修復一張圖片時，會一直反覆調用這個 pt 文件，迴圈中，調用 pt，然後需要再對調用結果進行相關處理步驟，直到一個清晰圖片出來才會結束循環，而這些處理步驟中可以穿插改動然後加入自己定義的新步驟，可以理解擴散模型的使用也有一套標準，但是可以按自己的理解去改動這些標準
...
**總結來說：**
PnP 算法之所以能夠結合先驗和後驗，是因為它們在擴散模型**多步疊代的逆向採樣過程**中：
...
這是一個從時間步 $T$ (純雜訊) 疊代到 0 (清晰圖像) 的過程。對於每一個時間步 $t$ (從 $T$ 遞減到 1)：
...
目前採樣速度是明顯缺陷，想改變這個缺陷，**是否可以加入 DDIM 等採樣器**，加速器本質上是為無條件生成設計了更高效的 SDE/ODE 離散化方案。要將它們與 DPS 結合，關鍵在於將 DPS 計算出的結合了似然梯度的“修正分數函數”或“修正噪聲預測”作為輸入，傳遞給這些加速採樣器的疊代更新公式。這意味著在 DDIM 的預測 $x_0$ 或者 DPM-Solver 的下一步 $x_{t−1}$ 計算中，需要將 DPS 特有的似然梯度貢獻考慮進去 。

**挑戰：確保加速採樣器在引入似然梯度後仍能保持穩定性和收斂性，以及在低NFE下重建質量不顯著下降。可能需要針對特定採樣器調整似然梯度項的權重或形式。**
...
> 
> 值得注意的是，儘管 PnP-DM 的 NFE 相對較高（在某些情況下約為 DPS 的 3 倍），但其運行時間卻與 DPS 相當（約為 DPS 的 1.5 倍）。這主要是因為 PnP-DM 透過在每次疊代中調整起始雜訊水平來避免運行完整的擴散過程，並且使用了耦合參數的退火排程，從而有效降低了實際運行時間。此外，PnP-DM 中每個先驗步驟的函數評估次數不一定為 100，因為隨著演算法運行，耦合參數 'p' 會減小，導致後期疊代中的步驟數遠少於 100。

##### 6.2.2.2 改進近似項

在DPS中有可能有以下部分這個部分：

- **僅依賴後驗均值：** 目前的近似只考慮了後驗均值 $\hat{x}_0$，而忽略了後驗分布 $p(x_0∣x_t)$ 的其他統計信息（可能是方差？）。

**挑戰： 估計高維分布的協方差通常計算成本很高，或者需要更複雜的神經網絡結構。如何將其高效且穩定地融入到DPS的疊代過程中是一個挑戰。**

##### 6.2.2.3 優化超參數

步長 $\zeta_i$ 在 DPS 算法中是很重要的超參數，論文通過消融實驗表明 $\zeta$ 的選擇對結果影響很大，並給出了一個經驗範圍 $[0.1, 1.0]$。然而，這種經驗性設定可能不適用於所有任務或噪聲水平。

我們是否可以在擴散過程中結合當前圖像的噪聲水平、與數據流形的距離、重建誤差等動態調整 $\zeta_i$？
